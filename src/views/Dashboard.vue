        ...
        <div v-if="canAccessInventory" class="dashboard-section">
          <div class="section-header">
            ...
            <button
              ...
              @click="handleDownloadAllHistory"
              :disabled="isDownloadingReport"
              title="Tải báo cáo toàn bộ giao dịch ra file Excel"
            >
              <Download
                class="action-icon-sm"
                :class="{ spin: isDownloadingReport }"
              />
              {{
                isDownloadingReport
                  ? 'Đang kết xuất...'
...
import { inventoryService } from '../services/inventory'; // Import Inventory API

import AdminLayout from '../layouts/AdminLayout.vue';
import DefaultLayout from '../layouts/DefaultLayout.vue';

// IMPORT THÊM Download
import {
  Users,
  ShieldCheck,
  Key,
...
// THÊM BIẾN NÀY ĐỂ KIỂM TRA SHIPPER
...
// LOGIC TẢI BÁO CÁO TỔNG HỢP EXCEL CHỐNG LỖI
const handleDownloadAllHistory = async () => {
  isDownloadingReport.value = true;
  toast.info('Đang kết xuất toàn bộ dữ liệu, vui lòng chờ...');

  try {
    const response = await inventoryService.exportAllWarehouseHistory();

    // 1. LẤY ĐÚNG DỮ LIỆU BLOB (Phòng ngừa Axios Interceptor can thiệp)
    const blobData =
    ...
    // 2. KIỂM TRA NẾU BACKEND TRẢ VỀ LỖI JSON (Do không có dữ liệu hoặc Server lỗi)
    if (blobData.type && blobData.type.includes('application/json')) {
      const text = await blobData.text();
      const errorObj = JSON.parse(text);
      toast.error(`Từ chối tải: ${errorObj.detail || 'Có lỗi từ máy chủ'}`);
      return; // Dừng lại, không tải file rác về máy!
    }

    // 3. TẠO FILE EXCEL CHUẨN
    const finalBlob = new Blob([blobData], {
    ...
    link.setAttribute('download', `Tong_Hop_Giao_Dich_Kho_${dateStr}.xlsx`);
    ...
    toast.error('Có lỗi xảy ra khi xuất báo cáo. Vui lòng kiểm tra lại!');
  } finally {
...
const canApproveTransfer = computed(() => {
  return (
    isAdmin.value ||
    authStore.hasPermission('FUNC_CUSTOMER_MGR') ||
    authStore.hasPermission('FUNC_TRUONG_KHO')
  );
});

const canAccessStats = computed(() => {
  const statPermissions = ['FUNC_THUONG_NHAP', 'FUNC_THUONG_XUAT'];
  return (
    isAdmin.value ||
...
const inventoryCards = computed(() => [
  ...
  {
    title: 'Duyệt Luân Chuyển',
    description: 'Xác nhận và cập nhật tồn kho các lô hàng chuyển nội bộ.',
    route: '/inventory/approve-transfer',
    icon: ClipboardCheck,
...
const masterDataCards = [
  ...
  },
  {
    ...
    description: 'Quản lý mã sản phẩm, tên sản phẩm và trọng lượng.',
  ...
  {
    title: 'Xe Vận Chuyển',
    description: 'Quản lý xe vận chuyển',
    route: '/admin/cars',
    icon: Van,
    colorClass: 'bg-emerald-light text-emerald',
  },
  {
    ...
    description: 'Quản lý đối tác vận chuyển và thiết lập bảng giá.',
    route: '/admin/shipping-partners',
    icon: Truck,
...
/* 🟢 STYLE CHO NÚT TẢI EXCEL MỚI */
.btn-export-all {
  display: flex;
  align-items: center;
  gap: 6px;
  background-color: #10b981; /* Xanh lá để thể hiện hành động Excel */
  color: white;
  border: none;
  padding: 6px 14px;
  border-radius: 8px;
